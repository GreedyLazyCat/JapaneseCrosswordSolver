...
class JCross(
     ...
     * Под фигурой я тут понимаю закрашенные подряд квадраты
     ...
     *
     * Так же есть очень важная проверка соответствия всех фигур кроме последней подсказке.
     * Например: подсказка - [1, 2, 3], считанные фигуры - [2, 1],
     * 2 != 1 (убираем из списка фигур последнюю, подсказки обрезаем так же, остается [1] и [2]) => отбрасываем вариант.
     * Это ускорило решение кроссворда crosses/bug_really_slow.json с больше чем часа, до 30 секунд!!!
     * (ускорение наблюдается и у других кроссвордов)
     */
    fun isColValid(
        col: Int,
...
//            }
    ...
    /**
     * Основной подход, который я использую - это перебор вариаций строк с отсечением вариантов,
     * грубо нарушающих строение столбцов до перебираемой в данный момент строки,
     * где-то это значительно ускоряет перебор, например в кроссворде crosses/cup.json
     * без отсечений решение занимает около часа, с отсечениями - минута.
     * Так же перед началом решения закрашиваются квадраты в столбцах, где это можно сделать
     * однозначно: пересечения, однозначно закрашиваемые столбцы.
     * Описание принципа отсечения в методе isColValid
     */
